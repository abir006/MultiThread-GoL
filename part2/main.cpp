...
static void calc_and_append_statistics2(const vector<tile_record>& tile_hist) {
    ifstream ifile(DEF_RESULTS_FILE_NAME);
    bool file_exists = ifile.good();
    ifile.close();

    std::ofstream results_file(DEF_RESULTS_FILE_NAME,
                               std::ofstream::app | std::ofstream::out);

    if (!file_exists) {
        results_file << "Tile_id,Thread_id,Compute_Time" << endl;
        // cout << "Successfully created results file: " << DEF_RESULTS_FILE_NAME << endl;
    }
    for(uint i=0 ; i<tile_hist.size() ; ++i) {
        uint tile_id = i;
        uint thread_id = tile_hist[i].thread_id;
        double computer_time = tile_hist[i].tile_compute_time;
        results_file << tile_id << "," << thread_id << "," << computer_time << endl;
    }
    results_file.close();
}
void run_scatter_plot() {
        game_params params = {4, 10,
                              "mid.txt",
                              false, false};

        Game g(params);
        g.run();
        calc_and_append_statistics2(g.tile_hist());
    }
void run_graphs() {
    for (uint n = 1; n <= 20; ++n) {
